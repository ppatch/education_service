...
router.get('/weixin', function (request, response,next) {
    ...
    if (code != undefined && code != null) {
            ...
            res.on('data', function (data) {
                ...
                https.get(getInfoUrl, function (res) {
                    ...
                    res.on('data', function (d2) {
                        ...
                        response.writeHead(302, { "Location": "http://huyugui.f3322.org:8101/#/home?"+ querystring.stringify({Openid: openid,name: _name,headimgurl: _headimgurl,message: result}) });
                        response.end();
                        // });
                    });
                });
        ...
        response.writeHead(302, {"Location": "http://huyugui.f3322.org:8101/#/home?params=xx"});
        response.end();
    }
})
...
router.get('/message', function (req, res, next) {
    ...
    Message.find({openid:req.query.openid})
        .sort({date: -1}).exec(function (error, result) {
            if(error!=null){next(error)}
            else {
                res.json(result);
